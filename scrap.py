...
"""
This is a developer tools
scrap unit infomation from http://wikiwiki.jp/aigiszuki/
"""
PATH = "xxxx.html"
RARE = "sapphire"
...
def __gen_unit_info_by_rare(path, rare):
    ...
    with open(path, "r") as f:
        contents = f.read()
        # start scrap
        unit_container = BeautifulSoup(contents, "lxml").findAll(
            "div", {"class": "splitinclude-included-page-container"}
        )
        aigis_unit_list = []
    ...
    if response.status_code == 200:
        ...
        for li_content in raw_content:
            if "/☆4金" in li_content.text:
                counter = counter + 1
                if counter <= 3:
                    awakening_material = awakening_material + li_content.text.split(
                        "/"
                    )[1].replace("☆4金", "")
                    if counter <= 2:
...
def __gen_unit_awakening_material():
    ...
    for aigis_unit in aigis_unit_list:
        ...
        while awakening_material is None:
            awakening_material = __scrap_unit_awakening_material(
                "https://wikiwiki.jp" + aigis_unit.info_url
            )
        data = {
...
def resolve_property_response(
    response, identifier, identify_id, update_field, group=""
):
    raw_content = (
        BeautifulSoup(response.text, "html.parser")
        .find(identifier, {"id": identify_id})
        .findNext("div")
        .find("tbody")
        .findAll("tr")
    )
    for row in raw_content:
        ...
        elif first_column_th is not None and first_column_th.text in [
            "近",
            "遠",
            "遠近",
        ]:
            ...
            for td in row.findAll("td"):
                ...
                if td_value is not None:
                    if td_value in ["", "編集", "\u3000"]:
                        ...
                        # 遠近距離更新
                        update_group(
                            unit_name_list, "property_distance", property_distance
                        )
...
def __gen_group():
    ...
    if response.status_code == 200:
        ...
        # 属性-特性
        resolve_property_response(
            response, "h3", "h3_content_2_2", "property_speciality"
        )
        # 属性-季節
        resolve_property_response(response, "h3", "h3_content_2_3", "property_season")
        # 男性
        resolve_property_response(
            response, "h4", "h4_content_2_5", "property_sex", "男"
        )
        # 特定能力
        resolve_property_response(
            response, "h4", "h4_content_2_6", "property_qualification"
        )
        # コラボ
        raw_content = (
            BeautifulSoup(response.text, "html.parser")
            .find("h3", {"id": "h3_content_1_5"})
            .findNext("div")
            .find("tbody")
            .findAll("tr")
        )
